...
public:
    ...
    Result<void> open(const std::string& portName, int baudRate,
                       ...
                       size_t readBufferSize = 65536) override {
        if (m_isOpen) return Result<void>::Error(ErrorCode::TransportAlreadyOpen);
    ...
    Result<size_t> write(const uint8_t* data, size_t size,
        ...
        if (!m_isOpen) return Result<size_t>::Error(ErrorCode::TransportNotOpen);
        if (m_cancelled) return Result<size_t>::Error(ErrorCode::TransportAsyncCancelled);
    ...
    Result<void> flush() override {
        if (!m_isOpen) return Result<void>::Error(ErrorCode::TransportNotOpen);
        m_records.push_back({CallRecord::Flush, 0});
        if (m_failOnFlush) return Result<void>::Error(ErrorCode::TransportWriteFailed);
        return Result<void>::Ok();
    }

    std::string backendName() const noexcept override { return "MockSerialBackend"; }
    ...
    // Access last config
    std::string lastPortName() const { return m_lastPortName; }
    int lastBaudRate() const { return m_lastBaudRate; }
...
private:
    ...
    std::string m_lastPortName;
    int m_lastBaudRate{0};
